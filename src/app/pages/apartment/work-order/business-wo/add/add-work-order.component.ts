...
export class AddWorkOrderComponent implements OnInit {
    ...
    next() {
        switch (this.current) {
            case 0:
                if (this.radioValue) {
                    this._houseParams.apartmentId = this.radioValue;
                    this.current += 1;
                    this.changeContent();
                } else {
                    this.message.create('warning', "未选择公寓品牌");
                }
                break;
            case 1:
    ...
    getApartmentList(data) {
        this._loading = true;
        this.apartmentService.getApartmentList(data).then(res => {
            this._loading = false;
    ...
    /**提交工单操作 */
        ...
        this.submitParams.apartmentId = this.radioValue;
    ...
    ngOnInit() {
        this.getApartmentList(this._params);
    }
}
