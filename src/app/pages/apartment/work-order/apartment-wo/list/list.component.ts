...
import { ApartmentWoDetailComponent } from "../detail/detail.component";
import { ApartmentWoAddComponent } from "../add/add.component";
...
export class ApartmentWoComponent implements OnInit {

  _params = {
		query: { apartmentId:sessionStorage.getItem('userId'),status: "", startTime: "", endTime: "", address: "" },
		page: {
			currentPage: 1,
			pageSize: 10
		}
	...
	//to详情
	goDetail(data) {
    this.router.navigate(['apartmentWo/detail/detail',data.id]);
	...
	addWorkOrder() {
		this.router.navigate(['apartmentWo/add/add']);
	}
