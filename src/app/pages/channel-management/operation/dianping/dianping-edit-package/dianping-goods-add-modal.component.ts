...
@Component({
    ...
    templateUrl: './dianping-goods-add-modal.component.html',
    styleUrls: ['./dianping-goods-add-modal.component.css']
})
export class DianpingGoodsAddModalComponent implements OnInit {
    constructor(
	...
	@Input() 
	set _packageId(value: string) {
		this.packageId =  value;
		this._params.packageId = this.packageId;
		this._virtualGoodsParams.packageId = this.packageId;
	}
	...
	refreshData(reset = false) {
		...
		this.operationListService
			.getDzListByExcludeGoodsPager(this._params)
			.then(resp => {
				this._loading = false;
	...
	searchVirtualGoods(reset = false) {
		...
		this.operationListService
			.getDzListByExcludeGoodsPager(this._virtualGoodsParams)
			.then(resp => {
	...
	selectRealGoods(item: any) {
		const itemCheckStatus = item.checked;
		this._dataSet.forEach(item => {
			item.checked = false;
		})
		item.checked = itemCheckStatus;
		this._refreshStatus();
	}

	selectVirtualGoods(item: any) {
		const itemCheckStatus = item.checked;
	...
	addGoods($event: MouseEvent) {
        ...
        if (this.currentTabIndex === 0) {
            this._dataSet.forEach(item => {
                if (item.checked) {
                    selectedItems.push({
						channelPackageId: this.packageId,
						channelGoodsId: item.id,
						orderAllocationBase: 0
					  })
                }
            })
        } else {
        ...
        this.operationListService.batchSavePackageGoods(selectedItems).then(resp => {
